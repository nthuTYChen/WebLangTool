...
Template.userIndex.events({
	...
	'submit form': function(event) {
		...
		Meteor.callAsync('serverWindow', {funcName: 'addUpdateProfile', info: newProfile});
	},
	'click button#assignLecturer': function() {
		let name = document.getElementById('lecturerName').value;
		Meteor.callAsync('serverWindow', {
	...
	'click button#testFuncCall': function() {
		Meteor.callAsync('serverWindow', {funcName: 'testFuncCall'});
	},
	'click button#printProfile': function() {
		...
		Meteor.callAsync('serverWindow', {funcName: 'printProfile', info: profile}).then((res)=>{
			console.log(res);
		}).catch((err)=>{
...
Template.vocab.events({
	...
	'submit form': function(event) {
		...
		answers.name = name;
		Meteor.callAsync('serverWindow', {funcName: 'checkAns', info: answers});
	}
});
